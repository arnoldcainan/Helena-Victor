...
document.addEventListener('click',event=>{const control=event.target.closest('[data-action]');if(!control)return;const action=control.dataset.action;if(action==='open-upload')openUpload();else if(action==='close-upload')closeUpload();else if(action==='share-album')shareAlbum(control.dataset.shareUrl);else if(action==='select-category')selectCategory(control);else if(action==='load-new-photos')loadNewPhotos();else if(action==='choose-photo')choosePhotoSource(control.dataset.inputId);else if(action==='close-lightbox')closeLightbox();else if(action==='close-lightbox-backdrop'&&event.target===control)closeLightbox();else if(action==='toggle-password'){const input=document.getElementById(control.dataset.targetInput);if(input){const isPassword=input.type==='password';input.type=isPassword?'text':'password';control.setAttribute('aria-pressed',isPassword?'true':'false');control.setAttribute('title',isPassword?'Ocultar senha':'Ver senha');control.setAttribute('aria-label',isPassword?'Ocultar senha':'Ver senha')}}});
...
document.body.addEventListener('htmx:configRequest',event=>{const cookieMatch=document.cookie.match(/csrftoken=([^;]+)/);const token=cookieMatch?cookieMatch[1]:(document.querySelector('[name=csrfmiddlewaretoken]')?.value||'');if(token&&!event.detail.headers['X-CSRFToken']){event.detail.headers['X-CSRFToken']=token}});
